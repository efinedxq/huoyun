...
function change2(v){
	var str="0";
	for(i=0;i<v;i++){
		str+=("_"+(document.getElementById(e[i]).selectedIndex-1));
	};
	var ss=document.getElementById(e[v]);
	with(ss){
		length = 0;
		options[0]=new Option(opt0[v],opt0[v]);
		if(v && document.getElementById(e[v-1]).selectedIndex>0 || !v){
			if(dsy.Exists(str)){
				ar = dsy.Items[str];
				for(i=0;i<ar.length;i++){
					options[length]=new Option(ar[i],ar[i]);
...
var e=["e_province","e_city","e_county"];//三个select的name
...
function _init_area(){  //初始化函数
	...
	for(i=0;i<s.length-1;i++){
	  document.getElementById(e[i]).onchange=new Function("change2("+(i+1)+")");
	}
	change1(0);
	change2(0);
}
function _init(){  //初始化函数
	for(i=0;i<s.length-1;i++){
	  document.getElementById(s[i]).onchange=new Function("change1("+(i+1)+")");
	}
	change1(0);
