...
<body>
	...
	<table align="center" cellspacing="1" width="720px">
		<tr>
			<td colspan="9" style="font-size: 14pt; color: #b11a55"><b>车源信息管理</b></td>
		</tr>
		<tr>
	...
	<table border="1px" align="center" cellspacing="0" width="720px">
		...
		<s:iterator value="#request.truList" var="tru" status="st">
			<tr>
				<td><a target="_blank"
					href="<%=path %>/truInfo.action?id=${id}"><s:property
				...
				<td><a href="<%=path %>/truDel.action?id=${id}">删除</a></td>
				<td><a
					href="<%=path %>/man/truCheck.action?id=${id}&check=1">通过</a>/<a
					href="<%=path %>/man/truCheck.action?id=${id}&check=0">取消</a></td>
			</tr>
		</s:iterator>
		...
		<tr>
			<td colspan="9">第${pageNo}页&nbsp&nbsp&nbsp&nbsp 共 ${pageCount}页
				&nbsp&nbsp&nbsp&nbsp <c:if test="${pageNo!=1}">
					<a href="<%=path %>/man/truManList?pageNo=${pageNo-1}"> 上一页 </a>
				</c:if>&nbsp&nbsp&nbsp&nbsp <c:if test="${pageNo!=pageCount}">
					<a href="<%=path %>/man/truManList?pageNo=${pageNo+1}" />">下一页 
					</a>
				</c:if>
			</td>
		</tr>
